...
export function loadStoredColumnSelection(
  ...
  try {
    ...
    const valid = parsed.filter(
      (key): key is string => typeof key === 'string' && availableKeys.includes(key)
    );
...
export function pickRecordColumns<T extends Record<string, unknown>>(
  row: T,
  columns: string[]
...
export function pickDataColumns<T extends Record<string, unknown>>(
  data: T[],
  columns: string[]
): Record<string, unknown>[] {
